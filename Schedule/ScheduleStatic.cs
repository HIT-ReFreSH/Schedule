...
namespace HitRefresh.Schedule
{
    ...
    public static class ScheduleStatic
    {
        ...
        ///     老师的名字
        /// </summary>
        public static Regex TeacherNameRegex { get; } = new(@"^[\u4e00-\u9fa5^0-9]{2,4}$|^(\w+\s?)+$");

        /// <summary>
        ///     课程周数表达式
        /// </summary>
        public static Regex CourseTimeRegex { get; } = new(@"^\[(((\d+)|((\d+)\-(\d+)))(单|双)?(\|)?)+\](单|双)?$");

        /// <summary>
        ///     教室名称表达式
        /// </summary>
        public static Regex LocationRegex { get; } = new(@"^([\u4e00-\u9fa5]+|[A-Z]{1,2})\d{2,5}$");
